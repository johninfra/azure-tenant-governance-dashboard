...
async function loadTenantData(){
 ...
 try{
  ...
  // Activity telemetry: directory audit logs work on Entra ID Free. Graph sign-in
  // downloads require P1/P2, so sign-ins gracefully activate when the tenant is licensed.
  try{
   const auditBody=await api(base+'/auditLogs/directoryAudits?$top=100&$orderby=activityDateTime%20desc',gt);
   state.data.directoryAudits=auditBody.value||[];
  }catch(e){
   state.directoryAuditsUnavailable=true;
   state.directoryAuditsMessage=norm(e);
  }
  try{
   const signInBody=await api(base+'/auditLogs/signIns?$top=100&$orderby=createdDateTime%20desc',gt);
   state.data.signIns=signInBody.value||[];
  }catch(e){
   state.signInsUnavailable=true;
   const raw=norm(e);
   state.signInsMessage=/premium|p1|p2|license|aadiam/i.test(raw)
    ? 'Microsoft Graph sign-in downloads require Microsoft Entra ID P1 or P2. Directory audit activity is shown below until the tenant is upgraded.'
    : raw;
  }
  ...
  try{
   let failures=0;
   const pairs=await mapLimit(users,4,async user=>{
    try{return [user.id,await paged(`${base}/users/${user.id}/authentication/methods`,gt)]}
    catch(e){failures++;return [user.id,null]}
   });
   state.data.userAuthMethods=Object.fromEntries(pairs.filter(([,methods])=>Array.isArray(methods)));
   if(!Object.keys(state.data.userAuthMethods).length){
    state.authMethodsUnavailable=true;
    state.authMethodsMessage='Registered authentication methods could not be read for tenant users. Confirm UserAuthenticationMethod.Read.All has admin consent and the signed-in account has a supported directory role.';
   }else if(failures){
    state.warnings.push(`Registered authentication methods could not be read for ${failures} user${failures===1?'':'s'}; available users are still shown.`);
   }
  }catch(e){state.authMethodsUnavailable=true;state.authMethodsMessage=norm(e)}
  try{
   state.data.authMethodsPolicy=await api(base+'/policies/authenticationMethodsPolicy',gt);
  }catch(e){state.authPolicyUnavailable=true;state.authPolicyMessage=norm(e)}
  try{
  ...
  if(guid(state.config.subscriptionId||'')){
   try{
    ...
    const [assignments,definitions,resourceGroups,azureResources,virtualNetworks,networkInterfaces,publicIpAddresses,networkSecurityGroups]=await Promise.all([
     ...
     paged(`${arm}/resources?api-version=2021-04-01`,at),
...
function buildFindings(){
 ...
 if(!state.data.mfa.length&&Object.keys(state.data.userAuthMethods||{}).length){
  for(const u of state.data.users.filter(x=>x.accountEnabled!==false&&x.userType!=='Guest'&&Object.prototype.hasOwnProperty.call(state.data.userAuthMethods,x.id))){
   if(!authMethodsForUser(u.id).some(isStrongAuthMethod)) add('medium','Enabled member has no registered strong authentication method',`${u.displayName} has no strong/MFA-capable authentication method returned by Microsoft Graph. Verify registration and access policy.`,u.userPrincipalName,`auth-method-member:${u.id}`);
  }
  for(const role of state.data.directoryRoles){
   if(!sensitive.test(role.displayName||''))continue;
   for(const member of role.members||[]){
    if(Object.prototype.hasOwnProperty.call(state.data.userAuthMethods,member.id)&&!authMethodsForUser(member.id).some(isStrongAuthMethod)) add('high','Privileged Entra role member has no registered strong authentication method',`${member.displayName} is an active member of ${role.displayName}, and no strong/MFA-capable authentication method was returned for the account.`,member.userPrincipalName||member.id,`auth-method-role:${role.id}:${member.id}`);
   }
  }
 }
...
function authMethodsForUser(id){return Array.isArray(state.data.userAuthMethods?.[id])?state.data.userAuthMethods[id]:[]}
function authMethodLabel(method={}){
 const t=String(method['@odata.type']||'').toLowerCase();
 if(t.includes('microsoftauthenticator'))return 'Microsoft Authenticator';
 if(t.includes('fido2')||t.includes('passkey'))return 'FIDO2 / Passkey';
 if(t.includes('windowshelloforbusiness'))return 'Windows Hello for Business';
 if(t.includes('softwareoath'))return 'Software OATH';
 if(t.includes('phoneauthentication'))return method.phoneType?(`Phone / ${method.phoneType}`):'Phone / SMS';
 if(t.includes('temporaryaccesspass'))return 'Temporary Access Pass';
 if(t.includes('x509certificate'))return 'Certificate-based authentication';
 if(t.includes('emailauthentication'))return 'Email';
 if(t.includes('passwordauthentication'))return 'Password';
 const raw=String(method['@odata.type']||'Authentication method').replace('#microsoft.graph.','').replace(/AuthenticationMethod$/,'');
 return raw.replace(/([a-z])([A-Z])/g,'$1 $2')||'Authentication method';
}
function isStrongAuthMethod(method={}){
 const t=String(method['@odata.type']||'').toLowerCase();
 return /microsoftauthenticator|fido2|passkey|windowshelloforbusiness|softwareoath|phoneauthentication|temporaryaccesspass|x509certificate/.test(t);
}
function usersWithAuthData(){return state.data.users.filter(u=>Object.prototype.hasOwnProperty.call(state.data.userAuthMethods||{},u.id))}
function usersWithStrongAuth(){return usersWithAuthData().filter(u=>authMethodsForUser(u.id).some(isStrongAuthMethod))}
function authPolicyConfigs(){return state.data.authMethodsPolicy?.authenticationMethodConfigurations||[]}
function authPolicyLabel(id=''){
 const k=String(id).toLowerCase();
 const names={microsoftauthenticator:'Microsoft Authenticator',sms:'SMS',fido2:'FIDO2 / Passkey',temporaryaccesspass:'Temporary Access Pass',softwareoath:'Software OATH',voice:'Voice',email:'Email OTP',x509certificate:'Certificate-based authentication',qrcodepin:'QR code + PIN'};
 return names[k]||String(id).replace(/([a-z])([A-Z])/g,'$1 $2');
}
function authPolicyTargets(cfg={}){
 const inc=cfg.includeTargets||[];
 if(inc.some(t=>String(t.id||'').toLowerCase()==='all_users'))return 'All users';
 const groups=inc.filter(t=>String(t.targetType||'').toLowerCase()==='group');
 if(groups.length){
  const byId=new Map(state.data.groups.map(g=>[g.id,g.displayName]));
  return groups.map(g=>byId.get(g.id)||g.id).join(', ');
 }
 return inc.length?`${inc.length} included target${inc.length===1?'':'s'}`:'No included targets exposed';
}
...
function render(){
 ...
 <nav class="nav">${nav('overview','Overview')}${nav('groups','Groups & Members')}${nav('privileged','Privileged Access')}${nav('mfa','MFA posture')}${nav('roles','Entra roles')}${nav('activity','Recent Activity')}${nav('topology','Azure Topology')}${nav('rbac','Azure RBAC')}${nav('findings','Findings')}${nav('setup','Setup')}</nav>
 ${renderTab()}
...
function renderTab(){
 ...
 return ({groups:renderGroups,privileged:renderPrivileged,mfa:renderMfa,roles:renderRoles,activity:renderActivity,topology:()=>renderTopology(state,{esc,badge,toolbar}),rbac:renderRbac,findings:renderFindings}[state.tab]||renderOverview)();
}
function renderOverview(){
 const p=mfaPct(),authUsers=usersWithAuthData().length,strongUsers=usersWithStrongAuth().length,strongPct=authUsers?strongUsers/authUsers*100:NaN,high=state.data.findings.filter(f=>f.severity==='high').length;
 ...
 ${stat('Strong auth',pct(strongPct),authUsers?`${strongUsers}/${authUsers} users with a registered strong/MFA-capable method`:state.data.mfa.length?`${state.data.mfa.filter(x=>x.isMfaCapable).length}/${state.data.mfa.length} MFA-capable in premium report`:'method data unavailable')}
 ...
 ${stat('Azure resources',state.data.azureResources.length,`${state.data.virtualNetworks.length} VNets • ${state.data.resourceGroups.length} resource groups`)}
 ${stat('Review score',score(),`${high} high-severity findings`)}
 </div><div class="grid two-col">
 <div class="card"><div class="section-title"><h2>MFA & authentication posture</h2><button class="btn" data-tab="mfa">View methods</button></div>${authUsers?`<div class="hero-num">${pct(strongPct)}</div><div class="progress"><span style="width:${Math.max(0,Math.min(100,strongPct))}%"></span></div><div class="muted small">${strongUsers}/${authUsers} users have at least one registered strong/MFA-capable method • ${authPolicyConfigs().filter(x=>String(x.state).toLowerCase()==='enabled').length} tenant methods enabled</div>`:state.data.mfa.length?`<div class="hero-num">${pct(p)}</div><div class="progress"><span style="width:${Math.max(0,Math.min(100,p))}%"></span></div><div class="muted small">Premium registration report available</div>`:`<div class="mfa-unavailable"><div class="mfa-icon">i</div><div><strong>Authentication method data unavailable</strong><p>${esc(state.authMethodsMessage||state.mfaMessage||'No authentication-method data was returned.')}</p></div></div>`}</div>
...
function renderMfa(){
 ...
 const users=state.data.users.filter(u=>{
  const methods=authMethodsForUser(u.id);
  const hay=`${u.displayName||''} ${u.userPrincipalName||''} ${methods.map(authMethodLabel).join(' ')}`.toLowerCase();
  return !q||hay.includes(q);
 }).sort((a,b)=>(a.displayName||'').localeCompare(b.displayName||''));
 const policy=authPolicyConfigs().slice().sort((a,b)=>authPolicyLabel(a.id).localeCompare(authPolicyLabel(b.id)));
 const enabled=policy.filter(x=>String(x.state).toLowerCase()==='enabled').length;
 const authUsers=usersWithAuthData().length,strongUsers=usersWithStrongAuth().length;
 return `<section>
  <div class="card"><div class="section-title"><div><h2>Tenant authentication methods policy</h2><div class="muted small">What Microsoft Entra currently allows in this tenant</div></div><span class="muted small">${policy.length?enabled+' enabled of '+policy.length+' configured methods':'Policy not loaded'}</span></div>
   ${state.authPolicyUnavailable?`<div class="mfa-unavailable"><div class="mfa-icon">i</div><div><strong>Authentication methods policy unavailable</strong><p>${esc(state.authPolicyMessage)}</p><p class="muted small">Add delegated <code>Policy.Read.AuthenticationMethod</code>, grant admin consent, and sign in with a supported directory role.</p></div></div>`:policy.length?`<div class="table-wrap"><table><thead><tr><th>Method</th><th>State</th><th>Targeting</th></tr></thead><tbody>${policy.map(x=>`<tr><td><strong>${esc(authPolicyLabel(x.id))}</strong></td><td>${badge(String(x.state||'unknown').toUpperCase(),String(x.state).toLowerCase()==='enabled'?'good':'neutral')}</td><td>${esc(authPolicyTargets(x))}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">No authentication-method policy configurations were returned.</div>'}
  </div>
  <div class="card"><div class="section-title"><div><h2>User registered authentication methods</h2><div class="muted small">Live per-user Microsoft Graph authentication-method inventory</div></div><span class="muted small">${strongUsers}/${authUsers||state.data.users.length} users with a registered strong/MFA-capable method</span></div>
   ${toolbar('Search users, UPNs, or authentication methods…')}
   ${state.authMethodsUnavailable?`<div class="mfa-unavailable large"><div class="mfa-icon">i</div><div><strong>Registered authentication methods unavailable</strong><p>${esc(state.authMethodsMessage)}</p><p class="muted small">Add delegated <code>UserAuthenticationMethod.Read.All</code>, grant admin consent, then refresh the tenant data.</p></div></div>`:`<div class="table-wrap"><table><thead><tr><th>User</th><th>Account</th><th>Registered methods</th><th>Strong/MFA-capable method</th></tr></thead><tbody>${users.map(u=>{const known=Object.prototype.hasOwnProperty.call(state.data.userAuthMethods||{},u.id),methods=authMethodsForUser(u.id),labels=[...new Set(methods.map(authMethodLabel))],strong=methods.some(isStrongAuthMethod);return `<tr><td><strong>${esc(u.displayName||'Unnamed user')}</strong><br><span class="muted">${esc(u.userPrincipalName||'—')}</span></td><td>${badge(u.accountEnabled===false?'Disabled':'Enabled',u.accountEnabled===false?'high':'good')}</td><td>${known?esc(labels.join(', ')||'Password only / no additional method returned'):'<span class="muted">Not readable</span>'}</td><td>${known?badge(strong?'Yes':'No',strong?'good':'medium'):badge('Unknown','neutral')}</td></tr>`}).join('')||'<tr><td colspan="4" class="empty">No users match the current filter.</td></tr>'}</tbody></table></div>`}
  </div>
  ${state.mfaUnavailable?`<div class="notice info"><strong>Premium MFA registration report:</strong> ${esc(state.mfaMessage)} The dashboard is using the per-user authentication-method inventory above instead; it does not claim Microsoft\'s licensed <code>isMfaCapable</code> telemetry.</div>`:''}
 </section>`;
}
...
function formatActivityTime(v){
 if(!v)return '—';
 const d=new Date(v);return Number.isNaN(d.getTime())?String(v):d.toLocaleString();
}
function signInStatus(x){
 const code=Number(x?.status?.errorCode||0);
 return code===0?'Success':(x?.status?.failureReason||x?.status?.additionalDetails||('Error '+code));
}
function signInLocation(x){
 return [x?.location?.city,x?.location?.state,x?.location?.countryOrRegion].filter(Boolean).join(', ')||'—';
}
function auditActor(x){
 const u=x?.initiatedBy?.user;
 const a=x?.initiatedBy?.app;
 return {name:u?.displayName||a?.displayName||'System / service',upn:u?.userPrincipalName||u?.id||a?.servicePrincipalName||a?.appId||'',ip:u?.ipAddress||'—'};
}
function auditTargets(x){
 return (x?.targetResources||[]).map(t=>t?.userPrincipalName||t?.displayName||t?.id).filter(Boolean).slice(0,4).join(', ')||'—';
}
function renderActivity(){
 const q=state.search.toLowerCase();
 const signIns=(state.data.signIns||[]).filter(x=>!q||`${x.userDisplayName||''} ${x.userPrincipalName||''} ${x.appDisplayName||''} ${x.resourceDisplayName||''} ${x.ipAddress||''} ${signInStatus(x)}`.toLowerCase().includes(q));
 const audits=(state.data.directoryAudits||[]).filter(x=>{const a=auditActor(x);return !q||`${a.name} ${a.upn} ${x.activityDisplayName||''} ${x.category||''} ${x.result||''} ${auditTargets(x)}`.toLowerCase().includes(q)});
 const signInLive=(state.data.signIns||[]).length>0;
 return `<section>
  <div class="card">
   <div class="section-title"><div><h2>Recent identity activity</h2><div class="muted small">Read-only Microsoft Graph activity for this Entra tenant.</div></div>${signInLive?badge('P1/P2 sign-in telemetry active','good'):badge('Free-license fallback','neutral')}</div>
   ${toolbar('Search user, UPN, application, activity, IP, or result…')}
   ${signInLive
    ?'<div class="notice info"><strong>Live sign-in logs available.</strong> The table below shows the most recent sign-in events returned by Microsoft Graph.</div>'
    :`<div class="notice info"><strong>Sign-in telemetry not available through Graph yet.</strong> ${esc(state.signInsMessage||'Microsoft Entra ID P1 or P2 is required for Graph sign-in downloads.')} Recent directory audit activity is still available below.</div>`}
  </div>
  ${signInLive?`<div class="card"><div class="section-title"><div><h2>Recent tenant sign-ins</h2><div class="muted small">Newest 100 events requested from Microsoft Graph.</div></div><span class="muted small">${signIns.length}/${state.data.signIns.length} shown</span></div>
   <div class="table-wrap"><table><thead><tr><th>Time</th><th>User</th><th>UPN</th><th>Application</th><th>Status</th><th>IP</th><th>Location</th></tr></thead><tbody>
   ${signIns.map(x=>`<tr><td>${esc(formatActivityTime(x.createdDateTime))}</td><td><strong>${esc(x.userDisplayName||'—')}</strong></td><td>${esc(x.userPrincipalName||'—')}</td><td>${esc(x.appDisplayName||x.resourceDisplayName||'—')}</td><td>${Number(x?.status?.errorCode||0)===0?badge('Success','good'):badge('Failed','high')}<br><span class="muted small">${esc(signInStatus(x))}</span></td><td><code>${esc(x.ipAddress||'—')}</code></td><td>${esc(signInLocation(x))}</td></tr>`).join('')||'<tr><td colspan="7" class="empty">No sign-in events match the current filter.</td></tr>'}
   </tbody></table></div></div>`:''}
  <div class="card"><div class="section-title"><div><h2>Recent directory audit activity</h2><div class="muted small">Available on Entra ID Free. This is administrative/directory activity, not a substitute for sign-in telemetry.</div></div><span class="muted small">${audits.length}/${state.data.directoryAudits.length} shown</span></div>
   ${state.directoryAuditsUnavailable?`<div class="notice warn"><strong>Directory audit data unavailable:</strong> ${esc(state.directoryAuditsMessage)}</div>`:''}
   <div class="table-wrap"><table><thead><tr><th>Time</th><th>Actor</th><th>Actor UPN / ID</th><th>Activity</th><th>Category</th><th>Result</th><th>Target</th><th>IP</th></tr></thead><tbody>
   ${audits.map(x=>{const a=auditActor(x);return `<tr><td>${esc(formatActivityTime(x.activityDateTime))}</td><td><strong>${esc(a.name)}</strong></td><td>${esc(a.upn||'—')}</td><td>${esc(x.activityDisplayName||'—')}</td><td>${esc(x.category||'—')}</td><td>${String(x.result||'').toLowerCase()==='success'?badge('Success','good'):badge(x.result||'Unknown','neutral')}</td><td>${esc(auditTargets(x))}</td><td><code>${esc(a.ip)}</code></td></tr>`}).join('')||'<tr><td colspan="8" class="empty">No directory audit events match the current filter.</td></tr>'}
   </tbody></table></div>
  </div>
 </section>`;
}
...
function exportJson(){download('azure-governance-findings.json',JSON.stringify({generatedAt:new Date().toISOString(),tenantId:state.config.tenantId,subscriptionId:state.config.subscriptionId||null,summary:{users:state.data.users.length,groups:state.data.groups.length,mfaCapablePercent:mfaPct(),usersWithStrongRegisteredAuthMethod:usersWithStrongAuth().length,usersWithReadableAuthMethods:usersWithAuthData().length,entraRoleAssignments:roleMembers(),azureRbacAssignments:state.data.azureRoleAssignments.length,reviewScore:score()},findings:state.data.findings},null,2),'application/json')}
